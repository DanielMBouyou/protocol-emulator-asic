...
| Reproducible benchmark: a frozen, versioned specification that a second operator regenerates from the dataset and the documented method | |
...
| AG-8 | The reduction and selection method is documented such that a second operator, using only the dataset and the method description, regenerates the same benchmark subset. | The regeneration is performed by a verifier and its result recorded as an OBSERVATION. |
...
All entries were verified on 2026-09-15 against the cited URL and re-extracted on 2026-09-17 by two independent operators at [S00](PREBENCH_PLAN.md#s00-methodological-setup). That re-extraction corrected F1, F5, F6, F15, F16, T1, T2, T4, I1, I3 and I4, where the page did not support what the entry asserted; the largest single error was a design count of 64 in T4 that the sources print as 40. The corrections and the claim records behind them are in [knowledge/](knowledge/README.md). Status labels (CONFIRMED_FACT, MOVING_CONSTRAINT, OPEN_QUESTION) and their re-verification rule are defined in [EVIDENCE_POLICY.md](EVIDENCE_POLICY.md#status-labels). The identifiers F, T, I, CTR, OBS-B and OQ are bootstrap identifiers; at stage S00 each entry receives a canonical register identifier and this table records the mapping.
...
| Id | Status | Statement |
| --- | --- | --- |
| F1 | CONFIRMED_FACT | The post is titled "Can you design a chip? Announcing the protocol emulator ASIC competition", is published on the Jane Street Blog, is dated 2026-09-10 and is bylined to Benjamin Devlin and Anish Singhani. The page never prints a sentence naming Jane Street as the organizer; organizer identity rests on the masthead, the bylines, the address asic-competition@janestreet.com and the first-person "we" throughout, and is a reading across those, not a printed statement. It is not marked NEEDS-INTERPRETATION in any claim record, because no record uses that value at all; see [LIM-0002](knowledge/limitation-register.md#lim-0002-the-reading_confidence-field-does-not-use-its-own-vocabulary). |
| F2 | CONFIRMED_FACT | Task statement: "Design an open-source, general-purpose protocol emulator ASIC." |
| F3 | CONFIRMED_FACT | The organizer describes a protocol emulator as "a tiny CPU with an instruction set designed for reading pins, writing pins, counting cycles, and hitting timing precisely enough that you can implement a real protocol in firmware rather than in fixed logic." The whole span is the organizer's, including the contrast with fixed logic. Recorded as the organizer's framing; not a project decision. |
...
| F5 | CONFIRMED_FACT | "Start with UART, SPI, and I2C." "Stretch goals include low-speed USB and 10Mbit Ethernet." ("10Mbit" is one printed token.) "Other interesting protocols to consider": JTAG, SWD, PS/2, CAN bus. The same bullet list ends with a fifth bullet: "Show us anything else your architecture makes possible that we haven't thought of." |
| F6 | CONFIRMED_FACT and MOVING_CONSTRAINT | Two separate printed sentences: "We're targeting IHP's 130nm CMOS5L process through our friends at Tiny Tapeout." and "We're targeting the March 2027 CMOS5L shuttle, subject to the foundry schedule." The second does not print "Tiny Tapeout"; that the March 2027 shuttle is a Tiny Tapeout shuttle is a reading across two sentences, not a printed statement. |
...
| F13 | CONFIRMED_FACT | The page prints "We'll pay to tape out the most novel designs on a Tiny Tapeout shuttle." and that it is "particularly interested in projects with unique functionality, as well as those that demonstrate novel approaches to design and verification methodologies!" The printed subject is "We"; that it is Jane Street is the reading recorded in F1, not a substitution made here. No rubric, weights, jury or number of winners published ([OQ-12](#oq-12-judging-rubric-winners-and-weighting)). |
| F14 | CONFIRMED_FACT | Winners receive their fabricated chip mounted on a dev board. |
| F15 | CONFIRMED_FACT | The page prints: "We are excited to see the languages and verification techniques you use, including formal methods, random constrained tests, AI-assisted verification, and more." and "At Jane Street, we use Hardcaml to generate the RTL for our FPGA and ASIC designs." The page says "languages" rather than "HDL", adds "and more", and states enthusiasm rather than permission. Those are differences between the two printed sentences and the wording this entry carried before S00; they are not recorded as absence claims, and no absence claim for the tokens "HDL", "any" or "are welcome" exists. It nowhere states that Hardcaml is not required; whether any language or framework is required is [OQ-26](#oq-26-expected-verification-framework), not a fact. |
| F16 | CONFIRMED_FACT (statements the page prints as guidance; it nowhere prints that they are binding) | SRAM may be more area-efficient than flip-flops for instruction memory; run synthesis early, check mapped cell area, leave room for clock tree buffers and routing, then full place-and-route and timing. "If you have access to an FPGA, consider using it to test your RTL before the ASIC flow." is printed as a bullet of "The challenge", not among the area-budgeting paragraphs. "Getting started" additionally prints that the Tiny Tapeout documentation walks through the process end to end with free and open source tools, and "Start by getting a UART transmitter out of a pin. Then make it programmable." |

### Tiny Tapeout facts

| Id | Status | Statement | Source |
| --- | --- | --- | --- |
| T1 | CONFIRMED_FACT | The template info.yaml prints a pinout section listing ui[0] to ui[7] under "# Inputs", uo[0] to uo[7] under "# Outputs" and uio[0] to uio[7] under "# Bidirectional pins", preceded by "The pinout of your project. Leave unused pins blank. DO NOT delete or add any pins." The pins are printed individually, not in range notation. The file prints neither clk nor rst_n; it prints only the project field "clock_hz". The clk and active-low rst_n convention is printed on the Tiny Tapeout GPIO page, a different source. Its tile-size comment is one claim of [CTR-01](#recorded-contradictions). | https://github.com/TinyTapeout/ttihp-verilog-template/blob/cmos5l/info.yaml |
| T2 | CONFIRMED_FACT | The template workflow .github/workflows/gds.yaml prints "uses: TinyTapeout/tt-gds-action@ihp-cmos5l" with "pdk: ihp-sg13cmos5l", plus the precheck, gl_test and viewer sub-actions at the same ref, and "runs-on: ubuntu-24.04". The file contains no occurrence of LibreLane or OpenLane in either case; any attribution of the hardening tool would have to come from the tt-gds-action repository, which this project has not yet extracted. Template licence Apache-2.0. | Same repository, branch cmos5l |
...
| T4 | CONFIRMED_FACT | The ttihp0p4 page prints "Experimental shuttle", "Launched: 27 March 2026", "Submission closed: 28 March 2026" and "Submitted to IHP using sg13cmos5l 130nm open source PDK". The design count is 40, not the 64 this entry carried before S00: the page states it only in its HTML metadata, "40 designs, closed 2026-03-28", and the runs page prints the same 40 in its TTIHP0p4 row. As of 2026-09-17 no March 2027 CMOS5L shuttle is listed on the runs page; the target shuttle exists only as the organizer's stated target ([OQ-30](#oq-30-listing-of-the-march-2027-shuttle)). | https://tinytapeout.com/chips/ttihp0p4/ and https://tinytapeout.com/runs/ |

### IHP PDK facts

| Id | Status | Statement | Source |
| --- | --- | --- | --- |
| I1 | CONFIRMED_FACT | IHP Open PDK is Apache-2.0. The main-branch landing page prints "As of March 2023, this repository is targeting the SG13G2 process node", lists ihp-sg13g2 and no ihp-sg13cmos5l at top level, and contains no occurrence of SG13CMOS5L or of the PDK environment variable. The dev README prints "It also hosts the SG13CMOS5L process node, a CMOS-only variant with a reduced metal stack." and "Switch between the process nodes by setting the `$PDK` environment variable to `ihp-sg13g2` or `ihp-sg13cmos5l`." There ihp-sg13cmos5l occurs only as a value of that variable, never as a path, and the README prints no directory listing. The separate repository IHP-GmbH/ihp-sg13cmos5l prints clone steps that place its content inside a dev-branch checkout, so this entry no longer asserts a CMOS5L directory on the dev branch. | https://github.com/IHP-GmbH/IHP-Open-PDK and https://github.com/IHP-GmbH/IHP-Open-PDK/blob/dev/README.md |
...
| I3 | MOVING_CONSTRAINT | IHP's dev README states the open-source PDK content is "preview only" and not intended for production "at this moment". The statement about silicon validation is narrower than this entry carried before S00: it appears in the Capacitors bullet and is scoped to two devices, cap_cmomi and cap_cmomf, of which it says "Neither is validated on CMOS5L silicon yet and neither models corner or mismatch spread". The README prints no general statement that CMOS5L models as a whole are unvalidated. | https://github.com/IHP-GmbH/IHP-Open-PDK/blob/dev/README.md |
| I4 | CONFIRMED_FACT | The repository IHP-GmbH/ihp-sg13cmos5l is titled "IHP SG13CMOS5L PDK (M1-M4-TM1 stack)" and prints the warning "This repo is meant to be used **only** as a temporary storage during the development of the `build/compile` migration script for the sg13 cmos5l PDK." The README body contains no licence section and no copyright text, which is the only thing the claim records establish. An Apache-2.0 label appears on the repository page itself, but no claim record covers that label, so this entry asserts no licence for the repository; it is recorded as [OQ-31](#oq-31-licence-of-the-standalone-cmos5l-repository). | https://github.com/IHP-GmbH/ihp-sg13cmos5l |
...
What constitutes a complete submission: RTL, GDS, documentation, testbenches, or software, a toolchain and an instruction set document, none of which the organizer names as a deliverable, or a subset? The organizer's framing (F3) uses the words "firmware" and "an instruction set"; it names no toolchain and no document.
...
Under which licence is IHP-GmbH/ihp-sg13cmos5l released? Its README body carries no licence section and no copyright text, and no claim record covers the label shown on the repository page. Opened at S00 when the re-extraction found that the earlier Apache-2.0 attribution rested on no claim.
