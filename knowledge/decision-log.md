...
- **facts and observations relied on**: [OBS-0001](observation-register.md#obs-0001-first-double-extraction-of-the-bootstrap-statements), [OBS-0003](observation-register.md#obs-0003-second-double-extraction-under-the-amended-rule), [LIM-0002](limitation-register.md#lim-0002-the-reading_confidence-field-does-not-use-its-own-vocabulary), [LIM-0003](limitation-register.md#lim-0003-the-documents-the-s00-claims-were-read-from-were-never-retained).
...
## DEC-0014 Retention is S00 work, and the documents read on 2026-09-17 were never retained

- **date**: 2026-09-18
- **deciders**: OP-OWNER
- **stage**: S00
- **statement**: Four findings and one act.
  1. **There is no deadlock in the canonical rules.** G00 condition 1 does require the retention rule, because [DEC-0008](#dec-0008-meaning-of-instantiating-a-register-and-of-the-dry-run-entry) makes a source record half of the dry-run entry and the condition imports every rule of [EVIDENCE_POLICY.md](../EVIDENCE_POLICY.md) without carve-out. But nothing in the plan assigns retention to S01, and nothing conditions the opening of a stage on the passing of the previous gate. The plan's stage-start rule is about inputs, and the [slip rule](../PREBENCH_PLAN.md#slip-rule) says "the stage continues until the failing condition passes", which assigns the remedy to S00. The circularity existed only in the wording of three records that misdescribed the rules.
  2. **This record corrects [DEC-0012](#dec-0012-record-of-the-g00-evaluation)**, which it does not edit. DEC-0012 incorporates the "what each failing condition needs" section of [gate-g00.md](gate-g00.md), and that section names S01 as the venue for retention. That venue is unsupported. The same misstatement appears in the header of [source-register.md](source-register.md), in the thirteen `retained_copy` values, and in [LIM-0003](limitation-register.md#lim-0003-the-documents-the-s00-claims-were-read-from-were-never-retained). DEC-0012's verdict on condition 1 stands; only its stated remedy is corrected.
  3. **S01 is not barred by the failed gate**, only deferred in date. gate-g00.md says "S01 has not opened", which was true as a report of what was done and is not a rule. S01 stays closed for now because this task does not open it, not because G00 forbids it.
  4. **The documents the S00 claims were extracted from were never retained and their byte identity is unrecoverable.** Retaining today does not repair that and is not presented as repairing it. It gives the project a hashed copy from today onwards and lets each claim be checked against it.
  - **The act.** Each of the thirteen sources is retrieved as raw bytes under this decision, written to the primary-document store fixed by [DEC-0004](#dec-0004-s00-d4-register-format-storage-and-primary-document-store), and hashed. Each `retained_copy` field records the file, its sha256 and the date the copy was taken, which is not the date in `accessed_on`, so no record asserts that the bytes read on 2026-09-17 are the bytes retained on 2026-09-18. Each retrieval is logged in [query-log.md](query-log.md). No claim, no fact and no existing record is rewritten. The comparison of every claim against its retained copy is recorded as [OBS-0004](observation-register.md#obs-0004-re-check-of-every-s00-claim-against-its-retained-copy).
- **options considered**:
  - Defer retention to S01, as LIM-0003 said. Rejected: it is the venue error this decision corrects, and while the project treats the failed gate as holding S01 shut, it is also unreachable.
  - Amend G00 condition 1 so the retention rule falls outside what the dry-run entry must satisfy. Rejected: the plan's gate rule says "No partial credit and no waiver", and DEC-0012 already rejected amending a failing condition so that it passes.
  - Treat the live URL as the retained source. Rejected: the root-of-evidence rule names "retained copies of web pages", not the pages, and a live page can change between a claim and its check, which is the whole reason the rule exists.
  - Record the retention as of 2026-09-17 so that it matches the claims. Rejected: it backdates evidence and asserts an identity nobody established.
  - Amend the retention rule so that PENDING is acceptable until benchmark freeze. Rejected: it licenses the state the rule exists to prevent, with claims accumulating against pages that can change unobserved.
  - Retain only SRC-0004, since condition 1 names only the dry-run entry. Considered seriously, because the condition is scoped to one entry and this would be the smallest act that satisfies it. Rejected: it would pass the condition while leaving twelve included sources in breach of the rule the condition invokes, which is passing a test rather than meeting a requirement. Retaining all thirteen costs one retrieval each.
  - Write today's hash into `retained_copy` and say nothing about the date. Rejected after review: a record whose `accessed_on` is 2026-09-17 and whose `retained_copy` is a hash taken on 2026-09-18 asserts that the document has that identity on the earlier date, which is not known. The date is therefore recorded inside the field.
- **facts and observations relied on**: [OBS-0004](observation-register.md#obs-0004-re-check-of-every-s00-claim-against-its-retained-copy). No FACT is relied on; this is a method decision.
- **rationale**: The gate was right and the remedy was filed in the wrong place. Correcting the venue costs nothing and unblocks the condition; weakening the condition or backdating the evidence would cost the only thing the registers are for. The permanent loss, that the bytes behind 202 claims were never hashed, is recorded rather than papered over, because a reader needs to know that these claims rest on a copy taken a day after they were made.
- **scope**: the thirteen source records of S00, the retention venue wherever it is stated, and the stated remedy for G00 condition 1. It does not touch condition 3, and it does not open S01.
- **consequences**: the retention half of condition 1 is satisfied for the dry-run entry and for the other twelve sources. Condition 1 as a whole is re-evaluated separately in [gate-g00-condition-1.md](gate-g00-condition-1.md). Three claims are found not to be supported by the retained copy of the source they cite, and that is recorded in OBS-0004 and carried into LIM-0003 rather than fixed by editing the claims.
- **record repairs made by this decision**: LIM-0003 is restated and retitled, from "No primary document has a retained copy", which is no longer true, to what is now the limitation. Two links that pointed at the old heading, one in DEC-0012 and one in gate-g00.md, are repointed at the new one. Only the link targets change; no sentence of either record is altered, and neither record is otherwise edited.
- **revisit trigger**: a re-access whose hash differs from the one recorded here, which the [handling of revisions](../RESEARCH_METHOD.md#handling-of-revisions-and-versions) treats as a new version.
